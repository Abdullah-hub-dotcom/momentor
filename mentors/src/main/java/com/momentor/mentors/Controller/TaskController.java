...
public class TaskController {
    ...
    //Ai Task Edit By Mentor
    @PutMapping("/{taskId}/edit")
    @PreAuthorize("hasRole('MENTOR')")
    public TaskResponseDto editAiTask(
        ...
        if (task.isApproved()) {
            throw new RuntimeException("Approved tasks cannot be edited");
        }
    ...
    //After Edit Needed Then Only Approval.
    ...
    public ResponseEntity<?> approveTask(@PathVariable Long taskId) {
        ...
        if (!task.isAutoGenerated()) {
            return ResponseEntity.badRequest()
                    .body("Only AI tasks need approval");
        }
